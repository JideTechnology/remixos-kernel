 ...
 *
 * $Id: dhdu.c 536992 2015-02-24 22:02:03Z $
 */
...
cmd_t dhd_cmds[] = {
	...
	{"cma_mem_info", dhd_varstr, DHD_GET_VAR, 0,
	" To Display CMA Base Address and Size Allocated.\n"
	},
	{ NULL, NULL, 0, 0, NULL }
};
...
/* For set_var, this function concatenates multiple arguments into a single string */
static int
dhd_varstr(void *dhd, cmd_t *cmd, char **argv)
{
	int error;
	char *str;
	char argstr[DHD_IOCTL_SMLEN];
	int i = 0;
	int argstrsize = DHD_IOCTL_SMLEN - 1;
	int argLen = 0;
	int argstrlen = 0;
	...
	if (!*++argv) {
		...
		while (argv[i] != '\0') {
			...
			if (i > 0) {
				...
				/* Append a space between arguments */
				memset(argstr + argstrlen, 0x20, 1);
				argstrsize--;
				argstrlen++;
			}

			argLen = strlen(argv[i]);
			if (!(argLen < argstrsize))
				return BCME_BADARG;

			memcpy(argstr + argstrlen, argv[i], argLen);
			argstrsize -= argLen;
			argstrlen += argLen;
		...
		memset(argstr + argstrlen, 0x0, 1);
		argstrsize--;
		argstrlen++;

		/* iovar buffer length includes NUL */
		return dhd_var_setbuf(dhd, cmd->name, argstr, argstrlen);
	}
}
