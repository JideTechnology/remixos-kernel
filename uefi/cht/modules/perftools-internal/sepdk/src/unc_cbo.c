...
)
{
    ...
    pecb_unc = LWPMU_DEVICE_PMU_register_data(&devices[dev_idx])[cur_grp];
    if (pecb_unc && GLOBAL_STATE_current_phase(driver_state) == DRV_STATE_RUNNING) {
...
/*!
 * @fn          static VOID unc_cbo_skl_Write_PMU(VOID*)
 *
 * @brief       Initial write of PMU registers
 *              Walk through the enties and write the value of the register accordingly.
 *              When current_group = 0, then this is the first time this routine is called,
...
)
{
    ...
    FOR_EACH_REG_ENTRY_UNC(pecb_unc, dev_idx, i) {
         ...
         */
        if (ECB_entries_reg_id(pecb_unc,i) == SKL_CBO_PERF_GLOBAL_CTRL) {
...
#if defined(MYDEBUG)
        SEP_PRINT_DEBUG("unc_cbo_skl_Write_PMU Event_Data_reg = 0x%x --- value 0x%llx\n",
...
/*!
 * @fn         static VOID unc_cbo_skl_Disable_PMU(PVOID)
 *
 * @brief      Zero out the global control register.  This automatically disables the
 *             PMU counters.
 *
 ...
 */
static VOID
unc_cbo_skl_Disable_PMU (
    PVOID  param
)
{
    SYS_Write_MSR(SKL_CBO_PERF_GLOBAL_CTRL, 0LL);
...
/*!
 * @fn         static VOID unc_cbo_skl_Enable_PMU(PVOID)
 *
 * @brief      Set the enable bit for all the CCCR registers
 *
 * @param      None
...
)
{
    ...
    pecb_unc = LWPMU_DEVICE_PMU_register_data(&devices[dev_idx])[cur_grp];
    if (pecb_unc && GLOBAL_STATE_current_phase(driver_state) == DRV_STATE_RUNNING) {
        SYS_Write_MSR(SKL_CBO_PERF_GLOBAL_CTRL, ECB_entries_reg_value(pecb_unc,0));
    }
 ...
 * Initialize the dispatch table
 */
DISPATCH_NODE  skylakeunc_cbo_dispatch =
{
    NULL,                                // initialize
    NULL,                                // destroy
    unc_cbo_skl_Write_PMU,               // write
    unc_cbo_skl_Disable_PMU,             // freeze
    unc_cbo_skl_Enable_PMU,              // restart
    UNC_COMMON_MSR_Read_PMU_Data,        // read
    NULL,                                // check for overflow
    NULL,                                //swap group
    NULL,                                //read lbrs
