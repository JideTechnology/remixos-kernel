...
/* ------------------------------------------------------------------------- */
/*!
 * @fn extern U32 PCI_Read_Ulong_Valid(pci_address)
 *
 * @param    pci_address   - PCI configuration address
 * @param    invalid_value - invalid value to compare against
 *
 * @return  value at this location, or 0 if invalid
 *
 * @brief   Reads a ULONG from PCI configuration space
 *          Makes sure that it's not an invalid value based on
 *          invalid_value arg
 *
 */
extern U32
PCI_Read_Ulong_Valid (
    U32 pci_address,
    U32 invalid_value
)
{
    ...
    temp_ulong = PCI_Read_Ulong(pci_address);
    
    if (temp_ulong == invalid_value) {
        return 0;
    }
    return temp_ulong;
}
