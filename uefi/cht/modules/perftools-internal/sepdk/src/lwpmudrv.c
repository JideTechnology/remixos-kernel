...
extern OS_STATUS SOCPERF_Switch_Group (void);
    ...
    if (num_devices) {
        for(i = 0; i < num_devices; i++) {
            ...
            if (LWPMU_DEVICE_em_groups_count(&devices[i]) > 1) {
                LWPMU_DEVICE_cur_group(&devices[i])++;
                LWPMU_DEVICE_cur_group(&devices[i]) %= LWPMU_DEVICE_em_groups_count(&devices[i]);
                SEP_PRINT_DEBUG("lwpmudrv_Switch_Group - Swap Group to %d for device %d\n",LWPMU_DEVICE_cur_group(&devices[i]), i);
                if (pecb_unc && ECB_device_type(pecb_unc) == DEVICE_UNC_SOCPERF) {
                    SOCPERF_Switch_Group();
                }
            }
...
)
{
    ...
    if (DRV_CONFIG_event_based_counts(pcfg_unc)) {
        ...
        LWPMU_DEVICE_acc_per_thread(&devices[cur_device]) = CONTROL_Allocate_Memory(GLOBAL_STATE_num_cpus(driver_state) *
                                                                                sizeof(VOID *));
        if (!LWPMU_DEVICE_acc_per_thread(&devices[cur_device])) {
            SEP_PRINT_ERROR("lwpmudrv_Configure_Events_UNC: unable to allocate memory for LWPMU_DEVICE_acc_per_thread!\n");
            return OS_NO_MEM;
        }
        LWPMU_DEVICE_prev_val_per_thread(&devices[cur_device]) = CONTROL_Allocate_Memory(GLOBAL_STATE_num_cpus(driver_state) *
                                                                                     sizeof(VOID *));
        if (!LWPMU_DEVICE_prev_val_per_thread(&devices[cur_device])) {
            LWPMU_DEVICE_acc_per_thread(&devices[cur_device]) = CONTROL_Free_Memory(LWPMU_DEVICE_acc_per_thread(&devices[cur_device]));
            SEP_PRINT_ERROR("lwpmudrv_Configure_Events_UNC: unable to allocate memory for LWPMU_DEVICE_prev_val_per_thread!\n");
            return OS_NO_MEM;
        }
        for (i = 0; i < GLOBAL_STATE_num_cpus(driver_state); i++) {
    ...
    switch (cmd) {
        ...
        */
        ...
        case DRV_OPERATION_SET_GFX_EVENT:
            ...
            SEP_PRINT_DEBUG("lwpmudrv_Device_Control: enable_gfx=%d, gfx_offset=%d\n",
                    (int)DRV_CONFIG_enable_gfx(pcfg),
                    EVENT_DESC_gfx_offset(global_ec));
