...
/*!
 * @fn          static VOID unc_ncu_skl_Write_PMU(VOID*)
 *
 * @brief       Initial write of PMU registers
 *              Walk through the enties and write the value of the register accordingly.
 *              When current_group = 0, then this is the first time this routine is called,
 ...
 */
static VOID
unc_ncu_skl_Write_PMU (
    VOID  *param
)
{
    UNC_COMMON_MSR_Write_PMU(param, 
                             SKL_NCU_PERF_GLOBAL_CTRL,
                             0,
                             0,
                             NULL);
...
/*!
 * @fn         static VOID unc_ncu_skl_Disable_PMU(PVOID)
 *
 * @brief      Zero out the global control register.  This automatically disables the
 *             PMU counters.
 *
 ...
 */
static VOID
unc_ncu_skl_Disable_PMU (
    PVOID  param
)
{
    SYS_Write_MSR(SKL_NCU_PERF_GLOBAL_CTRL, 0LL); 

    return;
}

/*!
 * @fn         static VOID unc_ncu_skl_Enable_PMU(PVOID)
 *
 * @brief      Set the enable bit for all the CCCR registers
 *
 * @param      param - device index
 ...
 */
static VOID
unc_ncu_skl_Enable_PMU (
    PVOID   param
)
{
    unc_ncu_Enable_PMU(param, SKL_NCU_PERF_GLOBAL_CTRL);
 ...
 * Initialize the dispatch table
 */
DISPATCH_NODE  skylakeunc_ncu_dispatch =
{
    NULL,                          // initialize
    NULL,                          // destroy
    unc_ncu_skl_Write_PMU,         // write
    unc_ncu_skl_Disable_PMU,       // freeze
    unc_ncu_skl_Enable_PMU,        // restart
    UNC_COMMON_MSR_Read_PMU_Data,  // read
    NULL,                          // check for overflow
    NULL,
    NULL,
