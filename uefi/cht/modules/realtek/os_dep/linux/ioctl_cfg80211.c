...
struct cfg80211_bss *rtw_cfg80211_inform_connected_bss(_adapter *padapter, struct wlan_network *pnetwork)
{
	struct ieee80211_channel *notify_channel;
	struct cfg80211_bss *bss = NULL;
	//struct ieee80211_supported_band *band;
	...
	buf = _rtw_zmalloc(MAX_BSSINFO_LEN);
	if(!buf)
		goto exit;
...
exit:
	...
	if(buf)
		_rtw_mfree(buf, MAX_BSSINFO_LEN);
...
void rtw_cfg80211_indicate_connect(_adapter *padapter)
{
...

	{
		...
		if (_rtw_memcmp(scanned->network.MacAddress, pnetwork->MacAddress, sizeof(NDIS_802_11_MAC_ADDRESS)) == _TRUE
			...
			if (!rtw_cfg80211_inform_connected_bss(padapter,scanned)) {
				DBG_871X(FUNC_ADPT_FMT" inform fail !!\n", FUNC_ADPT_ARG(padapter));
			} else {
				//DBG_871X(FUNC_ADPT_FMT" inform success !!\n", FUNC_ADPT_ARG(padapter));
			}
...
check_bss:
	...
	{
		//Changes for update from 3.10->3.14 where sme_state no longer exists.
...
void rtw_cfg80211_indicate_disconnect(_adapter *padapter)
{
	...
	if (!padapter->mlmepriv.not_indic_disco) {
		...
		DBG_871X("fw_state:%x current_bss:%p\n", get_fwstate(pmlmepriv),
					pwdev->current_bss);

		if (pwdev->current_bss == NULL)
...
static int rtw_cfg80211_add_monitor_if(_adapter *padapter, char *name, struct net_device **ndev)
{
	...
	DBG_871X_LEVEL(_drv_always_,
		"%s: mon_ndev->netdev_ops=%p, rtw_cfg80211_monitor_if_ops=%p\n",
                 __func__, mon_ndev->netdev_ops, &rtw_cfg80211_monitor_if_ops);
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,37)) || defined(COMPAT_KERNEL_RELEASE)
	rtw_cfg80211_rx_mgmt(padapter, freq, 0, pmgmt_frame, frame_len, 0, GFP_ATOMIC);
#else
	cfg80211_rx_action(padapter->pnetdev, freq, pmgmt_frame, frame_len, GFP_ATOMIC);
#endif
}
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,37)) || defined(COMPAT_KERNEL_RELEASE)
	rtw_cfg80211_rx_mgmt(adapter, freq, 0, frame, frame_len, 0, GFP_ATOMIC);
#else
...
#else
/* Block of code obsoleted by function prototype for k3.14
 * with change to mgmt_tx func pointer using "struct cfg80211_mgmt_tx_params" */
...
#endif
#endif /* end of block of obsoleted code by function prototype change for k3.14 */
	u64 *cookie)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,6,0))
	struct net_device *ndev = wdev_to_ndev(wdev);
	...
	*cookie = (unsigned long) params->buf;
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,14,0))
	memcpy(orig_buf, params->buf, params->len);
#else
	memcpy(orig_buf, buf, len);
#endif

#ifdef CONFIG_DEBUG_CFG80211
	DBG_871X(FUNC_ADPT_FMT" params->len=%zu, ch=%d"
	#if (LINUX_VERSION_CODE < KERNEL_VERSION(3,8,0))
		", ch_type=%d"
		#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,37)) || defined(COMPAT_KERNEL_RELEASE)	
	...
	DBG_8192C("RTW_Tx:tx_ch=%d, da="MAC_FMT"\n", tx_ch, MAC_ARG(GetAddr1Ptr(params->buf)));
	#ifdef CONFIG_P2P
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,14,0))
	memcpy(params->buf, orig_buf, params->len);
#else
...
static int cfg80211_rtw_sched_scan_start(struct wiphy *wiphy,
	...
	/*
	* In Android L, request->match_sets is used to store all PNO AP ssid
	* (Include hidden ssid)
	* Therefore, rtw_android_cfg80211_pno_setup() use it for sched_scan
	*/
	ret = rtw_android_cfg80211_pno_setup(dev, request->match_sets,
                        request->n_match_sets, request->interval);
...
#if defined(CONFIG_PM) && (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 0, 0))
	//Changed from 3.10->3.14 to be a pointer, passing address needed
