...
//	__u8 rgb16mode = 0;
	...
	else if(type->mod == DI_UVCOMBINEDMB)
	{
		...
		di_dev->linestrd0.dwval = (((image_w0 + 0x1f)&0xffe0) - 0x1f)<<0x05;
		di_dev->linestrd1.dwval = (((((image_w1)<<1)+0x1f)&0xffe0) - 0x1f) << 0x05;
								...
								__di_src_type_t *in_type,  __di_out_type_t *out_type)
{
	...
	memcpy(&di_dev->ch0_horzcoef0, lan3coefftab32_left		+ch0h_fir_coef_addr, 256);
	memcpy(&di_dev->ch0_horzcoef1, lan3coefftab32_right		+ch0h_fir_coef_addr, 256);
	memcpy(&di_dev->ch0_vertcoef, lan2coefftab32			+ch0v_fir_coef_addr, 256);

	if((out_type->fmt == DI_OUTUVCYUV420) || (in_type->fmt == DI_INYUV420))
	{
		memcpy(&di_dev->ch1_horzcoef0, bicubic8coefftab32_left	+ch1h_fir_coef_addr, 256);
		memcpy(&di_dev->ch1_horzcoef1, bicubic8coefftab32_right	+ch1h_fir_coef_addr, 256);
		memcpy(&di_dev->ch1_vertcoef, bicubic4coefftab32		+ch1v_fir_coef_addr, 256);
	}
	else
	{
		memcpy(&di_dev->ch1_horzcoef0, lan3coefftab32_left		+ch1h_fir_coef_addr, 256);
		memcpy(&di_dev->ch1_horzcoef1, lan3coefftab32_right		+ch1h_fir_coef_addr, 256);
		memcpy(&di_dev->ch1_vertcoef, lan2coefftab32			+ch1v_fir_coef_addr, 256);
	}

	if(out_type->alpha_en && (in_type->mod == DI_INTERLEAVED))
	{
		memcpy(&di_dev->ch3_horzcoef0, bicubic8coefftab32_left	+ch3h_fir_coef_addr, 256);
		memcpy(&di_dev->ch3_horzcoef1, bicubic8coefftab32_right	+ch3h_fir_coef_addr, 256);
		memcpy(&di_dev->ch3_vertcoef, bicubic4coefftab32		+ch3v_fir_coef_addr, 256);
	}
...
__s32 DI_Set_Di_Ctrl(__u8 en, __u8 mode, __u8 diagintp_en, __u8 tempdiff_en)
{
	...
	di_dev->di_ctrl.bits.flag_out_en = (mode==3)?0:1;  //for flag out, default enable
