...
static long sun8_th_reg_to_temp(u32 reg_data)
{
	s32 t;
...
static u32 sun8_th_temp_to_reg(long temp)
{
	s32 reg;
...
static void ths_sensor_init(struct sunxi_ths_data *ths_data)///gai
{
	...
	reg_value = readl(ths_data->base_addr + THS_CTRL_REG);
	reg_value |= SENS0_ENABLE_BIT | SENS1_ENABLE_BIT;
	writel(reg_value, ths_data->base_addr + THS_CTRL_REG);

	thsprintk(DEBUG_INIT, "THS_CTRL_REG = 0x%x\n", readl(ths_data->base_addr + THS_CTRL_REG));
...
static void ths_sensor_exit(struct sunxi_ths_data *ths_data)
{
	...
	writel(0, ths_data->base_addr + THS_CTRL_REG);
	return;
}
...
static int sun8i_th_get_temp(struct sunxi_ths_controller *controller, u32 id, long *temp)
{
	u32 reg_data;
	long t = 0;
	...
	if(reg_data & (0x100<<id)){
		...
		thsprintk(DEBUG_DATA_INFO, "THS data%d = 0x%x, temp is %ld\n", id, reg_data,t);
		if(-40 > t || 180 < t)
			return -1;
		*temp = t;
		return 0;
...
static int sun8i_th_suspend(struct sunxi_ths_controller *controller)///gai
{
	struct sunxi_ths_data *ths_data = (struct sunxi_ths_data *)controller->data;
...
static int sun8i_th_resume(struct sunxi_ths_controller *controller)///gai
{
	struct sunxi_ths_data *ths_data = (struct sunxi_ths_data *)controller->data;
