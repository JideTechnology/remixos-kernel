...
#define BQ_CHARGE_CUR_DCP	2500

#define BQ_GPIO_MUX_SEL_PMIC	0
#define BQ_GPIO_MUX_SEL_SOC		1

#define charger_enable_gpio	363

static int otg_state = 0;
static struct bq24192_chip *chip_for_otg=NULL;
static int otgirq = -1;
			...
			struct bq24192_chip *chip, int val)
{
	...
	bq24192_enable_charging(chip, val);
#if 0
			...
			union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ENABLE_CHARGING:
		...
		ret = bq24192_read_reg(chip->client,
                                        BQ24192_INPUT_SRC_CNTL_REG);
		if ( ret < 0) {
			dev_err(&chip->client->dev, "register read failed:\n");
			val->intval = false;
			break;
		}
		if ( (ret | 0x2) || (ret | 0x5) || (ret | 0x6) ) {
			val->intval = true;
		}
		break;
	case POWER_SUPPLY_PROP_ENABLE_CHARGER:
...
static int check_cable_status(struct bq24192_chip *chip, int reg_stat)
{
	...
	chip->max_cc = cable_props.ma;
//	bq24192_set_inlmt(chip, chip->max_cc);
	if(chip->max_cc) {
		if(!wake_lock_active(&chip->wakelock))
			wake_lock(&chip->wakelock);
	} else {
...
static irqreturn_t bq24192_irq_thread(int irq, void *devid)
{
	...
	schedule_delayed_work(&chip->irq_wrkr, msecs_to_jiffies(250));
	return IRQ_HANDLED;
}
...
static void bq24192_init_config(struct bq24192_chip *chip)
{
        int ret;
        int regval,vindpm;

        /* init reg00 */
        /* disable HIZ mode; set vbus input volt limit 0100;keep vbus input current limit */
	regval = 0x20;
        ret = bq24192_write_reg(chip->client, BQ24192_INPUT_SRC_CNTL_REG,
                                regval);
        if (ret < 0)
                dev_err(&chip->client->dev, "%s %d failed\n", __func__,__LINE__);
        ...
        if (ret < 0)
                dev_warn(&chip->client->dev, "Input Src Ctrl reg read fail\n");
        regval = ret &(~0x21);
        ret = bq24192_write_reg(chip->client, BQ24192_POWER_ON_CFG_REG,
                                regval);
        if (ret < 0)
                dev_err(&chip->client->dev, "%s %d failed\n", __func__,__LINE__);

        /* init reg02 */
        /* set fastcharging = 2560mA; */
        regval = 0x80;
        ...
        /* init reg03 */
        /* set pre-charging limit 256mA,set termination limit 256 mA */
        regval = 0x11;
        ret = bq24192_write_reg(chip->client, BQ24192_PRECHRG_TERM_CUR_CNTL_REG,
                                regval);
        if (ret < 0)
                dev_err(&chip->client->dev, "%s %d failed\n", __func__,__LINE__);

        /* init reg04 */
        /* set battery charger voltage 4.35V(0xD6). bat_low = 3.0V, recharger = 100mV */
        regval = 0xD6;
        ...
        /* init reg05 */
        /* enable terminal,enable fastcharging timer,timer = 12hrs,disable i2cWDT */
        regval= bq24192_read_reg(chip->client, BQ24192_CHRG_TIMER_EXP_CNTL_REG);
        if ((regval & 0x30) > 0) {
                dev_err(&chip->client->dev, "ERROR: i2c wdt need disable first,now is 0x%x\n",regval);
        }
        regval=0x8C;
        ret = bq24192_write_reg(chip->client, BQ24192_CHRG_TIMER_EXP_CNTL_REG,
                                regval);
        if (ret < 0)
                dev_err(&chip->client->dev, "%s %d failed\n", __func__,__LINE__);

        /* init reg06 */
        /* set boostv = 4.998V,Thermal Regulation Threshold = 120 */
        regval = 0x73;
        ret = bq24192_write_reg(chip->client, BQ24192_CHRG_THRM_REGL_REG,
                                regval);
        if (ret < 0)
                dev_err(&chip->client->dev, "%s %d failed\n", __func__,__LINE__);
        ...
        /* init reg07 */
        regval = 0x4B;
        ret = bq24192_write_reg(chip->client, BQ24192_MISC_OP_CNTL_REG,
                                regval);
        if (ret < 0)
                dev_err(&chip->client->dev, "%s %d failed\n", __func__,__LINE__);
...
#if 0
static int notify_otg_coming(struct bq24192_chip *chip, int otg)
{
	if(otg)
	{
		printk("TSOS notifiy the otg IN info\n");
		atomic_notifier_call_chain(&chip->transceiver->notifier,
                                USB_EVENT_ID,
                                NULL);
		irq_set_irq_wake(otgirq, 1);
	}
	else
	{
		printk("TSOS notifiy the otg OUT info\n");
		atomic_notifier_call_chain(&chip->transceiver->notifier,
                                USB_EVENT_NONE,
                                NULL);
		irq_set_irq_wake(otgirq, 0);
	}
	//Notes: Use charger to give out the disconnect notifier!
	return 0;
}

static irqreturn_t otg_wake_isr(int irq, void *devid)
{
	printk("TSOS irq=%d ,otgirq=%d\n", irq, otgirq);

	irq_set_irq_type(otgirq, otg_state ? IRQF_TRIGGER_LOW|IRQF_SHARED : IRQF_TRIGGER_HIGH|IRQF_SHARED);
	if(otg_state)
		otg_state=0;
	else
		otg_state=1;
...
{
	//struct bq24192_chip *chip = (struct bq24192_chip *)devid;
	struct bq24192_chip *chip = chip_for_otg;
	printk("TSOS %s\n", __func__);
	schedule_delayed_work(&chip->otg_wrkr, 0);
	return IRQ_WAKE_THREAD;
}

static void otg_irq_worker(struct work_struct *work)
{
#if 0
	struct bq24192_chip *chip = container_of(work,
						    struct bq24192_chip,
			...
			const struct i2c_device_id *id)
{
	...
	gpiod_direction_output(gpio_to_desc(charger_enable_gpio),1);
        mdelay(1);
	...
	//INIT_DELAYED_WORK(&chip->otg_wrkr, otg_irq_worker);
...
#if 0
		{
			//Notes: Add OTG ajudgement.
			int err;
		        err = gpio_request_one(CAMELLIA_OTG_ID,GPIOF_DIR_IN,"irq_for_otg");
			if(err)
			{
				printk("TSOS set irq ret=%d\n",err);
			}
			else
			{

			        printk("TSOS set irq for usb otg  \n");
				err = gpiod_direction_input(gpio_to_desc(CAMELLIA_OTG_ID));
				printk("Set irq direction ret=%d\n",err);
				msleep(100);

				otgirq = gpio_to_irq(CAMELLIA_OTG_ID);
				chip_for_otg = chip;
				ret = request_threaded_irq(otgirq, otg_wake_isr, otg_irq_thread, IRQF_TRIGGER_LOW|IRQF_SHARED, "otg_wake", chip);

				printk("TSOS otg irq request ret=%d\n",ret);
				if(ret==0)
					irq_set_irq_wake(otgirq, 1);
				//Notes: OTG not goto sleep
