...
static int bq2419x_init(struct bq2419x_chip *bq2419x)
{
	...
	if (bq2419x->in_current_limit == CURRENT_LIMIT)
...
static int bq2419x_charger_init(struct bq2419x_chip *bq2419x)
{
	...
	/* Configure Output Current Control to 2.048A*/
	ret = regmap_write(bq2419x->regmap, BQ2419X_CHRG_CTRL_REG, bq2419x->reg02);
	if (ret < 0) {
	...
	ret = regmap_update_bits(bq2419x->regmap, BQ2419X_TIME_CTRL_REG,
				BQ2419X_CHAGRE_TIMER_MASK, BQ2419X_CHAGRE_TIMER_8H);
		if (ret < 0) {
			dev_err(bq2419x->dev,
				"TIME_CTRL_REG write failed: %d\n", ret);
			return ret;
		}

	/*
	 * Configure Input voltage limit reset to OTP value,
	 * and charging current to 2A.
	 */
	ret = regmap_write(bq2419x->regmap, BQ2419X_INPUT_SRC_REG, bq2419x->reg00);
	if (ret < 0)
		dev_err(bq2419x->dev, "INPUT_SRC_REG write failed %d\n", ret);
		...
		if ((val & BQ2419x_CHRG_STATE_MASK) ==
			...
			if (bq2419x->update_status)
				bq2419x->update_status
					(bq2419x->status, (val & BQ2419x_VBUS_STAT));
		...
		struct bq2419x_platform_data *pdata)
{
	...
	bq2419x->otg_usb_vbus = pdata->vbus_pdata->otg_usb_vbus;
...
scrub:
	...
	gpio_free(bq2419x->otg_usb_vbus);
	return ret;
}
...
static int disable_charge(struct bq2419x_chip *bq2419x)
{
	unsigned int val;
	int ret;
...
static int enable_charge(struct bq2419x_chip *bq2419x)
{
	unsigned int val;
	int ret;
...
{  
        if(atomic_read(&mChargeEnable)) sprintf(buf,"%s","1"); 
...
{ 
			...
			struct bq2419x_chip *bq2419x=i2c_get_clientdata(client);
				...
				if(!strncmp(buf, "0", 1)){
					if(atomic_read(&mChargeEnable)==1)
						{
							atomic_set(&mChargeEnable,0);
							disable_charge(bq2419x);
						}
					}
				if(!strncmp(buf, "1", 1)){
					if(atomic_read(&mChargeEnable)==0)
						{
							atomic_set(&mChargeEnable,1);
							enable_charge(bq2419x);
						}
					}
...
static ssize_t charge_led_show(struct device *dev, struct kobj_attribute *attr,char *buf )  
{
	struct i2c_client *client = to_i2c_client(dev);
	struct bq2419x_chip *bq2419x=i2c_get_clientdata(client);
	unsigned int val;
	...
	printk("BQ2419X_PWR_ON_REG val is 0x%x\n",val);
	if(val & 0x30)
	{
		ret = regmap_read(bq2419x->regmap, BQ2419X_TIME_CTRL_REG, &val);
		if (ret < 0) {
			dev_err(bq2419x->dev, "FAULT_REG read failed: %d\n", ret);
			goto error1;
		}
		printk("BQ2419X_TIME_CTRL_REG val is 0x%x\n",val);
		if(val & 0x40) {
			return sprintf(buf, "%d\n", 1); 
		}
		else
...
static ssize_t charge_led_store(struct device *dev,struct kobj_attribute *attr,const char* buf, size_t count)  
{ 
	struct i2c_client *client = to_i2c_client(dev);
	struct bq2419x_chip *bq2419x=i2c_get_clientdata(client);
	int ret;

	printk("charge_led_store\n");

	if(strncmp(buf, "1", 1) == 0)
	{
		printk("set to green led\n");
		
		ret = regmap_update_bits(bq2419x->regmap, BQ2419X_PWR_ON_REG,
				 BQ2419X_ENABLE_CHARGE_MASK, BQ2419X_DISABLE_CHARGE);
		if (ret < 0) {
			printk("BQ2419X_PWR_ON_REG update failed: %d\n", ret);
			goto error1;
		}
				...
				const struct i2c_device_id *id)
{
	...
	if (pdata->bcharger_pdata) {
		...
		bq2419x->reg00 = pdata->bcharger_pdata->bq24190_reg00;
		bq2419x->reg02 = pdata->bcharger_pdata->bq24190_reg02;
	}
...
static void bq2419x_shutdown(struct i2c_client *client)
{
	...
	if (alarm_time && (bq2419x->in_current_limit > CURRENT_LIMIT)) {
	...
	if (alarm_time && (bq2419x->in_current_limit <= CURRENT_LIMIT)) {
		ret = regmap_write(bq2419x->regmap,
				BQ2419X_INPUT_SRC_REG, bq2419x->reg00);
		if (ret < 0)
			dev_err(bq2419x->dev,
				"INPUT_SRC_REG write failed %d\n", ret);
	}
