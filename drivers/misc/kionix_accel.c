...
#define __KIONIX_ACC_DEV__
#ifdef __KIONIX_ACC_DEV__
...
typedef union {
	...
	s32 v[SENSOR_DATA_SIZE];
} kionix_raw_data;
static kionix_raw_data kionix_offset;
static struct kionix_accel_driver *flash_priv;
static int kionix_major = 240;
static int kionix_minor = 0;
static struct cdev kionix_cdev;
static struct class *kionix_class = NULL;
static dev_t kionix_dev;
static char kionix_OffsetFileName[] = "/forever/misc/gsensor/offset.txt";
static int kionix_Device_First_Time_Opened_flag=1;
//#define KIONIX_DEBUG_DATA
#ifdef KIONIX_DEBUG_DATA
...
#define DEFAULT_SENSITIVITY 980
	...
	if(atomic_read(&acceld->accel_enabled) > 0) {
		if(atomic_read(&acceld->accel_enable_resume) > 0)
		{
			...
			if (err < 0) {
				...
				#ifdef __KIONIX_ACC_DEV__
				acceld->accel_data[acceld->axis_map_x] -= kionix_offset.v[0];
				acceld->accel_data[acceld->axis_map_y] -= kionix_offset.v[1];
				acceld->accel_data[acceld->axis_map_z] -= kionix_offset.v[2];
				#endif
...
static void kionix_acc_get_acceleration_data(struct kionix_accel_driver *acceld)
{
	struct { union {
		s16 accel_data_s16[3];
		s8	accel_data_s8[6];
	...
	if(atomic_read(&acceld->accel_enabled) > 0) {
		if(atomic_read(&acceld->accel_enable_resume) > 0)
		{
			...
			while(loop) {
				mutex_lock(&input_dev->mutex);
				#if defined(CONFIG_MACH_T8400N_8_3CM) || defined(CONFIG_MACH_NV8IA)
				if(acceld->gsensor_type == KIONIX_ACCEL_WHO_AM_I_KXTIK_1004) {
					err = kionix_i2c_read(acceld->client, ACCEL_GRP2_XOUT_L, (u8 *)accel_data.accel_data_s16, 6);
				}
				else
				#endif
...
exit:
 ...
 #ifdef __KIONIX_ACC_DEV__
static void kionix_acc_read_avg(kionix_raw_data *avg_p )
{
   	s32 xyz_acc[SENSOR_DATA_SIZE];   
  	s32 xyz[SENSOR_DATA_SIZE];
  	int i, j;
	int err;
	...
	for(i = 0; i < AVG_NUM; i++) 
	{      
		kionix_acc_get_acceleration_data(flash_priv);
		for(j = 0; j < SENSOR_DATA_SIZE; ++j) 
			xyz_acc[j] += flash_priv->accel_data[j];
  	}

	// calculate averages
  	for(i = 0; i < SENSOR_DATA_SIZE; ++i) 
		avg_p->v[i] = (s32) (xyz_acc[i] / AVG_NUM);
}
/* calc delta offset */
static int kionix_acc_calculate_offset(int gAxis,kionix_raw_data avg)
{
	switch(gAxis)
	{
		case CONFIG_GSEN_CALIBRATION_GRAVITY_ON_Z_NEGATIVE:  
			kionix_offset.u.x =  avg.u.x ;    
			kionix_offset.u.y =  avg.u.y ;
			kionix_offset.u.z =  avg.u.z - DEFAULT_SENSITIVITY;
		  break;
		case CONFIG_GSEN_CALIBRATION_GRAVITY_ON_X_POSITIVE:  
			kionix_offset.u.x =  avg.u.x + DEFAULT_SENSITIVITY;    
			kionix_offset.u.y =  avg.u.y ;
			kionix_offset.u.z =  avg.u.z ;
		  break;
		...
		case CONFIG_GSEN_CALIBRATION_GRAVITY_ON_Y_NEGATIVE:
			kionix_offset.u.x =  avg.u.x ;    
			kionix_offset.u.y =  avg.u.y + DEFAULT_SENSITIVITY;
			kionix_offset.u.z =  avg.u.z ;
		  break;
...
void kionix_acc_calibrate(int side)
{	
	kionix_raw_data avg;
	IN_FUNC_MSG;
	// get acceleration average reading
	kionix_acc_read_avg(&avg);
	// calculate and set the offset
	kionix_acc_calculate_offset(side, avg); 
}

void kionix_acc_set_offset(int val[3])
{
	int i;
	IN_FUNC_MSG;
	for(i = 0; i < SENSOR_DATA_SIZE; ++i)
		kionix_offset.v[i] = (s32) val[i];
}
static void kionix_acc_write_offset_to_file(void)
{
	char data[21];
	unsigned int orgfs;
	long lfile=-1;

	sprintf(data,"%6d %6d %6d",kionix_offset.u.x,kionix_offset.u.y,kionix_offset.u.z);
	orgfs = get_fs();
// Set segment descriptor associated to kernel space
	set_fs(KERNEL_DS);
	
        lfile=sys_open(kionix_OffsetFileName,O_WRONLY|O_CREAT, 0777);
	if (lfile < 0)
	{
 	 printk("sys_open %s error!!. %ld    I will create it.\n",kionix_OffsetFileName,lfile);
	}
	else
	{
   	 sys_write(lfile, data,21);
	 sys_close(lfile);
	}
	set_fs(orgfs);
...
static void kionix_acc_read_offset_from_file(void)
{
	unsigned int orgfs;
	char data[21];
	long lfile=-1;
...
// Set segment descriptor associated to kernel space
	...
	lfile=sys_open(kionix_OffsetFileName, O_RDONLY, 0);
	if (lfile < 0)
	{
	 ...
	 if(lfile==-2)
	 {
	   ...
	   if(lfile >=0)
	   {
	    strcpy(data,"000000 000000 000000");
 	    printk("sys_open %s OK!!. %ld\n",kionix_OffsetFileName,lfile);
	    sys_write(lfile,data,21);
	    sys_read(lfile, data, 21);
	    sys_close(lfile);
	   }
	  else
 	   printk("sys_open %s error!!. %ld\n",kionix_OffsetFileName,lfile);
	...
	sscanf(data,"%6d %6d %6d",&kionix_offset.u.x,&kionix_offset.u.y,&kionix_offset.u.z);
	set_fs(orgfs);


}

static int kionix_open(struct inode *inode, struct file *filp)
{
	IN_FUNC_MSG;
	//Add by Steve Huang 2011-11-30
		if(kionix_Device_First_Time_Opened_flag)
	{
          kionix_Device_First_Time_Opened_flag=0;
	      kionix_acc_read_offset_from_file();
	}
	printk("device_open: open_from_Android\n");
	return 0; 
}

static ssize_t kionix_write(struct file *filp, const char *buf, size_t count, loff_t *f_pos)
{
	IN_FUNC_MSG;
...
static ssize_t kionix_read(struct file *filp, char *buf, size_t count, loff_t *f_pos)
{
...
static long kionix_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	int err = 0, ret = 0, i;
	...
	switch(cmd) 
	{
		...
		case SENSOR_CALIBRATION:
			...
			kionix_acc_calibrate(intBuf[0]);
			// save file 2011-11-30	
			kionix_acc_write_offset_to_file();
			
			//return the offset
			for(i = 0; i < SENSOR_DATA_SIZE; ++i)
				intBuf[i] = kionix_offset.v[i];
...
static int kionix_close(struct inode *inode, struct file *filp)
{
	IN_FUNC_MSG;
...
struct file_operations kionix_fops = 
{
	.owner = THIS_MODULE,
	.read = kionix_read,
	.write = kionix_write,
	...
	.open = kionix_open,
	.release = kionix_close,
};

static int kionix_acc_dev_init(struct kionix_accel_driver *acc)
{
	dev_t dev = MKDEV(kionix_major, 0);
	int alloc_ret = 0;
	int major;
	int cdev_err = 0;
	int input_err = 0;
	...
	alloc_ret = alloc_chrdev_region(&dev, 0, 1,KIONIX_ACCEL_NAME);
	if(alloc_ret) {
		goto error;
	}
	major = MAJOR(dev);
	...
	cdev_init(&kionix_cdev, &kionix_fops);
	kionix_cdev.owner = THIS_MODULE;
	kionix_cdev.ops = &kionix_fops;
	cdev_err = cdev_add(&kionix_cdev, MKDEV(kionix_major, kionix_minor), 1);
	if(cdev_err) {
		goto error;
	}

	// register class
	kionix_class = class_create(THIS_MODULE, KIONIX_ACC_CLASS_NAME);
	if(IS_ERR(kionix_class)) {
		goto error;
	}
	kionix_dev = MKDEV(kionix_major, kionix_minor);
	class_dev = device_create(kionix_class, NULL, kionix_dev, NULL, KIONIX_ACCEL_NAME);

	printk("============================================================\n");
	printk("kionix driver loaded\n");
	printk("============================================================\n");	
