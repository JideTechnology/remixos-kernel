...
static ssize_t elan_ls_device_id_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct elan_epl_data *epld = epl_data;
    long *tmp = (long*)buf;
    uint16_t mode =0;
...
#define PAGE_SIZE         4096
	...
	res = snprintf(buf, PAGE_SIZE,
		...
		"0x11:%8x\n"
		"0x13:%8x\n",
		i2c_smbus_read_byte_data(epld->client, 0x00), i2c_smbus_read_byte_data(epld->client, 0x08),
		i2c_smbus_read_byte_data(epld->client, 0x10), i2c_smbus_read_byte_data(epld->client, 0x18),
		i2c_smbus_read_byte_data(epld->client, 0x20), i2c_smbus_read_byte_data(epld->client, 0x28),
		i2c_smbus_read_byte_data(epld->client, 0x30), i2c_smbus_read_byte_data(epld->client, 0x38),
		i2c_smbus_read_byte_data(epld->client, 0x48), i2c_smbus_read_byte_data(epld->client, 0x68),
		i2c_smbus_read_byte_data(epld->client, 0x70), i2c_smbus_read_byte_data(epld->client, 0x71),
		i2c_smbus_read_byte_data(epld->client, 0x80), i2c_smbus_read_byte_data(epld->client, 0x88),
		i2c_smbus_read_byte_data(epld->client, 0x98));
	mutex_unlock(&als_get_sensor_value_mutex);
#undef PAGE_SIZE
...
static DEVICE_ATTR(elan_ls_operationmode, S_IRUSR|S_IWUSR, elan_ls_operationmode_show,elan_ls_operationmode_store);

static DEVICE_ATTR(elan_ls_sensor_als_ch0_rawdata, S_IRUSR|S_IWUSR, elan_ls_sensor_als_ch0_rawdata_show,NULL);
static DEVICE_ATTR(elan_ls_sensor_als_ch1_rawdata, S_IRUSR|S_IWUSR, elan_ls_sensor_als_ch1_rawdata_show,NULL);
static DEVICE_ATTR(elan_ls_als_lux, S_IRUSR|S_IWUSR, elan_ls_als_lux_show, NULL);

static DEVICE_ATTR(elan_ls_incandescent_threshold, S_IRUSR|S_IWUSR, elan_ls_incandescent_threshold_show, elan_ls_incandescent_threshold_store);
static DEVICE_ATTR(elan_ls_fluorescent_threshold, S_IRUSR|S_IWUSR, elan_ls_fluorescent_threshold_show, elan_ls_fluorescent_threshold_store);
static DEVICE_ATTR(elan_ls_mix_threshold, S_IRUSR|S_IWUSR, elan_ls_mix_threshold_show,elan_ls_mix_threshold_store);

static DEVICE_ATTR(elan_ls_als_ax2, S_IRUSR|S_IWUSR, elan_ls_als_ax2_show, elan_ls_als_ax2_store);
static DEVICE_ATTR(elan_ls_als_bx, S_IRUSR|S_IWUSR, elan_ls_als_bx_show, elan_ls_als_bx_store);
static DEVICE_ATTR(elan_ls_als_c, S_IRUSR|S_IWUSR, elan_ls_als_c_show, elan_ls_als_c_store);
static DEVICE_ATTR(elan_ls_als_ax, S_IRUSR|S_IWUSR, elan_ls_als_ax_show, elan_ls_als_ax_store);
static DEVICE_ATTR(elan_ls_als_b, S_IRUSR|S_IWUSR, elan_ls_als_b_show, elan_ls_als_b_store);
static DEVICE_ATTR(elan_ls_device_id, S_IRUSR, elan_ls_device_id_show, NULL);
...
static struct attribute *ets_attributes[] =
{
    ...
    &dev_attr_elan_ls_device_id.attr,
    NULL,
};
...
static int initial_sensor(struct elan_epl_data *epld)
{
    ...
    epld->enable_lflag = 0;//jimmy modify

    return ret;
}

#define DEVICE_NAME		"light"
static int light_major = 241;
static int light_minor = 0;
static struct cdev light_cdev;
static struct class *light_class = NULL;
static dev_t light_dev;
...
{		
	...
	alloc_ret = alloc_chrdev_region(&dev, 0, 1, ELAN_LS_6814);
	if(alloc_ret) {
		goto error;
	}
	major = MAJOR(dev);
	...
	cdev_init(&light_cdev, &elan_als_fops);
	light_cdev.owner = THIS_MODULE;
	light_cdev.ops = &elan_als_fops;
	cdev_err = cdev_add(&light_cdev, MKDEV(light_major, light_minor), 1);
	if(cdev_err) {
		goto error;
	}

	// register class
	light_class = class_create(THIS_MODULE, ELAN_LS_6814);
	if(IS_ERR(light_class)) {
		goto error;
	}
	light_dev = MKDEV(light_major, light_minor);
	class_dev = device_create(light_class, NULL, light_dev, NULL, ELAN_LS_6814);

	printk("============================================================\n");
	printk("light driver loaded\n");
	printk("============================================================\n");	
...
static int lightsensor_setup(struct elan_epl_data *epld)
{
    ...
    epld->als_input_dev->name = ELAN_LS_6814;
