...
#define SENSOR_NAME 	"SENSOR_CM36283"
...
struct CM36283_data {
	struct i2c_client *PS_CM36283_client;
	struct i2c_client *ALS_CM36283_client;
	atomic_t delay;
	int als_enable;
	int ps_enable;
...
static void CM36283_work_func(struct work_struct *work)
{
	...
	unsigned long delay = atomic_read(&CM36283->delay);
        if(delay>200)   delay=200;  //lzk
        delay= msecs_to_jiffies(delay);        

	ps  = CM36283_get_ps(CM36283->PS_CM36283_client);
	if(CM36283->ps_enable)
	{
               // printk("CM36283--lzk--ps=%d-- \n",ps); //lzk
		if (ps > -1) { 
			input_report_abs(CM36283->ps_input, ABS_DISTANCE, ps);  //lzk
			input_sync(CM36283->ps_input);
		}
	}
	als = CM36283_get_als(CM36283->ALS_CM36283_client);
	if(CM36283->als_enable)
	{
               // printk("CM36283--lzk--als=%d-- \n",als); //lzk
		if ((als > 0) || (als==0)) { // -1
                        if(als<5) als=5;
...
static int CM36283_als_state(struct CM36283_data *CM36283, int als_on)
{
	...
	rc = CM36283_smbus_write_word(CM36283->ALS_CM36283_client,ALS_THDL, 0x01F4);
	if(rc < 0)
		pr_err("%s CM36283_smbus_write_word rc=%d\n", __func__, rc);

	rc = CM36283_smbus_write_word(CM36283->ALS_CM36283_client,ALS_THDH, 0x07D0);
	if(rc < 0)
		pr_err("%s CM36283_smbus_write_word rc=%d\n", __func__, rc);

	rc = CM36283_smbus_write_word(CM36283->ALS_CM36283_client,ALS_CONF, reg);
	if(rc < 0)
		pr_err("%s CM36283_smbus_write_word rc=%d\n", __func__, rc);
...
static int CM36283_ps_state(struct CM36283_data *CM36283, int ps_on)
{
	...
	rc = CM36283_smbus_write_word(CM36283->PS_CM36283_client,PS_CONF1_CONF2, reg);
	if(rc < 0)
		pr_err("%s CM36283_smbus_write_word rc=%d\n", __func__, rc);

	rc = CM36283_smbus_write_word(CM36283->PS_CM36283_client,PS_THD, 0x0705);
	if(rc < 0)
		pr_err("%s CM36283_smbus_write_word rc=%d\n", __func__, rc);
...
static int PS_CM36283_config(struct CM36283_data *CM36283,struct i2c_client *client)
{
	int rc; 

	rc = CM36283_smbus_read_word(client,DEV_ID);  //CM36283->ALS_CM36283_
	if(rc < 0)
		pr_err("%s CM36283_smbus_read_word rc=%d\n", __func__, rc);
	else
		pr_info("%s DEV_ID=0x%x\n",__func__, rc);

	rc = CM36283_smbus_write_word(client,PS_CONF1_CONF2, PS_DISABLE);
	if(rc < 0)
		pr_err("%s CM36283_smbus_write_word rc=%d\n", __func__, rc);

	rc = CM36283_smbus_write_word(client,PS_THD, 0x0705);
	if(rc < 0)
		pr_err("%s CM36283_smbus_write_word rc=%d\n", __func__, rc);
...
static int CM36283_config(struct CM36283_data *CM36283,struct i2c_client *client)
{
	int rc; 
	...
	rc = CM36283_smbus_write_word(client,ALS_CONF, ALS_DISABLE);
	if(rc < 0)
		pr_err("%s CM36283_smbus_write_word rc=%d\n", __func__, rc);

	rc = CM36283_smbus_write_word(client,ALS_THDL, 0x01F4);
	if(rc < 0)
		pr_err("%s CM36283_smbus_write_word rc=%d\n", __func__, rc);

	rc = CM36283_smbus_write_word(client,ALS_THDH, 0x07D0);
	if(rc < 0)
		pr_err("%s CM36283_smbus_write_word rc=%d\n", __func__, rc);
		...
		const char *buf, size_t count)
{
	...
	if(data)
	{
		if(!CM36283->ps_enable)
		{
                        /*error = PS_CM36283_config(CM36283,CM36283->PS_CM36283_client);//lzk
			if(error < 0)
			{
				printk("--ps_enable CM36283_config error=%d\n", error);
				return count;
			}
                        msleep(100); */
		...
		struct device_attribute *attr, char *buf)
{
	...
	struct i2c_client *client = to_i2c_client(dev);
	struct CM36283_data *CM36283 = i2c_get_clientdata(client);
	data = CM36283_read_ps(CM36283->PS_CM36283_client);
	return sprintf(buf, "%d\n", data);
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	struct i2c_client *client = to_i2c_client(dev);
	struct CM36283_data *CM36283 = i2c_get_clientdata(client);
	data = CM36283_smbus_read_word(CM36283->PS_CM36283_client, CM36283_reg);
...
static int CM36283_input_init(struct CM36283_data *CM36283)
{
	...
	int err;
        ...
        ps_dev = input_allocate_device();  //lzk add ps 
	if (!ps_dev)
	{
		pr_err("%s error input_allocate_device\n", __func__);
		return -ENOMEM;
	}
	ps_dev->name ="SENSOR_CM36283";
	ps_dev->id.bustype = BUS_I2C;
        set_bit(EV_ABS, ps_dev->evbit);
	input_set_capability(ps_dev, EV_ABS, ABS_DISTANCE);
	input_set_capability(ps_dev, EV_ABS, ABS_MISC);
        //input_set_drvdata(ps_dev, CM36283);
	input_set_abs_params(ps_dev, ABS_DISTANCE, 0, PS_MAX, 0, 0);
	input_set_abs_params(ps_dev, ABS_MISC, 0, ALS_MAX, 0, 0);
	err = input_register_device(ps_dev);
	if (err < 0) {
		input_free_device(ps_dev);
		return err;
	}
	CM36283->ps_input = ps_dev;  
...
{
	struct input_dev *dev = CM36283->ps_input;
		...
		const struct i2c_device_id *id)
{
	...
	if (!lightps_data) {
                printk("--CM36283_probe--lightps_data==NULL--\n");
		err = -ENOMEM;
		goto exit;
	} 
	err = CM36283_input_init(lightps_data);
	if (err < 0)
		goto kfree_exit;
	i2c_set_clientdata(client, lightps_data);
	lightps_data->PS_CM36283_client = client;


	err = sysfs_create_group(&client->dev.kobj, &CM36283_attribute_group);

	if (err < 0)
		goto error_sysfs;

        if(prob_flag==2){
		printk("----i2c test error prob_flag==2\n");
		goto error_sysfs;
        }else{
                printk("----i2c test  prob_flag=%d--\n",prob_flag); //lzk
        }
         
        err = PS_CM36283_config(lightps_data,client);//lzk
	if((err < 0) && (prob_flag!=1))
	{
		printk("---CM36283_probe--i2c test error err=%d\n", err);
		goto error_sysfs;
	}
	//pr_info("-----------%s-----------oK!\n", __func__);
	return 0;
error_sysfs:
	CM36283_input_delete(lightps_data);
        sysfs_remove_group(&client->dev.kobj, &CM36283_attribute_group);
kfree_exit:
	//kfree(lightps_data);
exit:
	return err;
...
static int cm36283_create_i2c_device(void)            //lzk
{
        struct i2c_adapter *adapter;
        struct i2c_client *client;
        struct i2c_board_info info = {
                I2C_BOARD_INFO("SENSOR_CM36283", 0x70), //lzk 0x60
        };
        adapter = i2c_get_adapter(3);
        if (!adapter) {
                printk(KERN_ERR "-cm36283--i2c_get_adapter failed\n");
                return -EINVAL;
        }

        client = i2c_new_device(adapter, &info);
        if (!client) {
                printk(KERN_ERR "--cm36283--i2c_new_device() failed\n");
		...
		struct device_attribute *attr, char *buf)
{
	...
	struct i2c_client *client = to_i2c_client(dev);
	struct CM36283_data *CM36283 = i2c_get_clientdata(client);
	data = CM36283_read_als(CM36283->ALS_CM36283_client);
	return sprintf(buf, "%d\n", data);
}
static ssize_t CM36283_als_delay_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct CM36283_data *CM36283 = i2c_get_clientdata(client);
...
static ssize_t CM36283_als_delay_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
...
static DEVICE_ATTR(als_delay, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		CM36283_als_delay_show, CM36283_als_delay_store);
static struct attribute *ALS_CM36283_attributes[] = {
	&dev_attr_enable_als.attr,
	&dev_attr_als_data.attr,
...
static struct attribute_group ALS_CM36283_attribute_group = {
	.attrs = ALS_CM36283_attributes
};

static void ALS_CM36283_input_delete(struct CM36283_data *CM36283)
{
	struct input_dev *dev = CM36283->input;
	input_unregister_device(dev);
	input_free_device(dev);
}
static int ALS_CM36283_input_init(struct CM36283_data *CM36283)
{
...
static int ALS_CM36283_probe(struct i2c_client *client,
		const struct i2c_device_id *id)
{
	int err = 0;
	//struct CM36283_data *data;
	...
	//lightps_data = kzalloc(sizeof(struct CM36283_data), GFP_KERNEL);
	if (!lightps_data) {
                printk("--ALS_CM36283_probe--lightps_data==NULL--\n");
		err = -ENOMEM;
		goto exit;
	}
	err = ALS_CM36283_input_init(lightps_data);
	if (err < 0)
		goto kfree_exit;
	i2c_set_clientdata(client, lightps_data);
	lightps_data->ALS_CM36283_client = client;
	mutex_init(&lightps_data->value_mutex);

	INIT_DELAYED_WORK(&lightps_data->work, CM36283_work_func);
	atomic_set(&lightps_data->delay, CM36283_MAX_DELAY);

	//err = sysfs_create_group(&data->input->dev.kobj,&CM36283_attribute_group);
	err = sysfs_create_group(&client->dev.kobj,&ALS_CM36283_attribute_group);

	if (err < 0)
		goto error_sysfs;
	err = CM36283_config(lightps_data,lightps_data->ALS_CM36283_client);
	if(err < 0)
	{
                prob_flag=2;
		printk(" ALS_CM36283_probe--i2c error--CM36283_config error err=%d\n", err);
		goto error_sysfs;
	}
        prob_flag=1; //lzk
	//pr_info("-----------%s-----------oK!\n", __func__);
	return 0;
error_sysfs:
	ALS_CM36283_input_delete(lightps_data);
        sysfs_remove_group(&client->dev.kobj, &ALS_CM36283_attribute_group);
kfree_exit:
	//kfree(lightps_data);
exit:
	return err;
...
static int ALS_CM36283_remove(struct i2c_client *client)
{
	struct CM36283_data *data = i2c_get_clientdata(client);

	sysfs_remove_group(&client->dev.kobj, &ALS_CM36283_attribute_group);
	ALS_CM36283_input_delete(data);
	kfree(data);
	return 0;
}

static const struct i2c_device_id ALS_CM36283_id[] = {
	{ "CM36283_ALS", 0 },
	{ }
};
MODULE_DEVICE_TABLE(i2c, ALS_CM36283_id);

#ifdef CONFIG_PM
static int ALS_CM36283_suspend(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct CM36283_data *CM36283 = i2c_get_clientdata(client);
	int err = 0;
...
static int ALS_CM36283_resume(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct CM36283_data *CM36283 = i2c_get_clientdata(client);
	unsigned long delay = msecs_to_jiffies(atomic_read(&CM36283->delay));
...
static struct i2c_driver ALS_CM36283_driver = {
	.driver = {
		.owner	= THIS_MODULE,
		.name	= "CM36283_ALS",
#ifdef CONFIG_PM
		.pm = &ALS_CM36283_pm_ops,
#endif
	},
	.id_table	= ALS_CM36283_id,
	.probe		= ALS_CM36283_probe,
	.remove		= ALS_CM36283_remove,
};


static int als_cm36283_create_i2c_device(void)            //lzk
{
        struct i2c_adapter *adapter;
        struct i2c_client *client;
        struct i2c_board_info info = {
                I2C_BOARD_INFO("CM36283_ALS", 0x60), 
        };
        adapter = i2c_get_adapter(3);
        if (!adapter) {
                printk(KERN_ERR "-als-cm36283--i2c_get_adapter failed\n");
                return -EINVAL;
        }

        client = i2c_new_device(adapter, &info);
        if (!client) {
                printk(KERN_ERR "--als-cm36283--i2c_new_device() failed\n");
                i2c_put_adapter(adapter);
                return -EINVAL;
        }
        return 0;
...
static int __init CM36283_init(void)
{
        ...
        als_cm36283_create_i2c_device();
	i2c_add_driver(&ALS_CM36283_driver);

        msleep(25);
        cm36283_create_i2c_device();
	return i2c_add_driver(&CM36283_driver);
}

static void __exit CM36283_exit(void)
{
	i2c_del_driver(&ALS_CM36283_driver);
        i2c_del_driver(&CM36283_driver);
}


late_initcall(CM36283_init);
