...
int tegra_edid_get_monspecs(struct tegra_edid *edid, struct fb_monspecs *specs,struct tegra_dc_out *out)
{
	int i;
	int j;
	int ret;
	...
	kref_init(&new_data->refcnt);
	memset(edid_buf, 0x0, sizeof(edid_buf));
	new_data->support_stereo = 0;
	data = new_data->dc_edid.buf;

	if(out&&out->read_ext_edid){
		ret = out->read_ext_edid(edid_buf);
		if (ret)
			goto fail;
	}
	
	if(out&&out->read_ext_edid){
		memcpy(data, edid_buf, sizeof(edid_buf));
	}
