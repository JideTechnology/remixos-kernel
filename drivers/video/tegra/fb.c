...
static int tegra_fb_blank(int blank, struct fb_info *info)
{
	...
	case FB_BLANK_UNBLANK:
		...
		if (tegra_fb->win->dc->enabled &&
			...
			return 0;
		tegra_fb->win->flags |= TEGRA_WIN_FLAG_ENABLED;
		if(tegra_fb->win->dc->pdata->fb->flags & TEGRA_FB_FLIP_ON_PROBE_180)
			tegra_fb->win->flags |= (TEGRA_WIN_FLAG_INVERT_H | TEGRA_WIN_FLAG_INVERT_V);
	...
	case FB_BLANK_POWERDOWN:
		...
		if(!(tegra_fb->win->dc->pdata->fb->flags & TEGRA_FB_DISABLE_BLANK))
					...
					struct resource *fb_mem)
{
	...
	if(fb_data->flags & TEGRA_FB_FLIP_ON_PROBE_180)
		win->flags |= (TEGRA_WIN_FLAG_INVERT_H | TEGRA_WIN_FLAG_INVERT_V);
