...
{
	struct palmas_irq_chip_data *data = g_chip_data;
	if(data){
		printk("%s:========> disable_mask=%x\n",__func__,mask);
		//data->disable_mask = mask;
	}
	
}
EXPORT_SYMBOL_GPL(palmas_disable_irq_work);


static void id_ota_irq_work(struct work_struct *work)
{
	struct palmas_irq_chip_data *d = container_of(
			work, struct palmas_irq_chip_data, works[ID_OTG_IRQ_INDEX-20]);

	printk("%s:=========>\n",__func__);
	handle_nested_irq(761);

}

static void id_irq_work(struct work_struct *work)
{
	struct palmas_irq_chip_data *d = container_of(
			work, struct palmas_irq_chip_data, works[ID_IRQ_INDEX-20]);

	printk("%s:=========>\n",__func__);
	handle_nested_irq(762/*irq_find_mapping(d->domain, ID_IRQ_INDEX)*/);

}
static void vbus_otg_irq_work(struct work_struct *work)
{
	struct palmas_irq_chip_data *d = container_of(
			work, struct palmas_irq_chip_data, works[VBUS_OTG_IRQ_INDEX-20]);

	printk("%s:=========>\n",__func__);
	handle_nested_irq(763);

}

static void vbus_irq_work(struct work_struct *work)
{
	struct palmas_irq_chip_data *d = container_of(
			work, struct palmas_irq_chip_data, works[VBUS_IRQ_INDEX-20]);

	printk("%s:=========>start\n",__func__);
	handle_nested_irq(764/*irq_find_mapping(d->domain, VBUS_IRQ_INDEX)*/);

}
static void id_ota_irq_timer(unsigned long _data)
{
	struct palmas_irq_chip_data *d = (void *)_data;
	printk("%s:=========>\n",__func__);
	if(d->disable_mask&ID_OTG_IRQ_FLAG)return;
	queue_work(d->irq_workq,&d->works[ID_OTG_IRQ_INDEX-20]);

}

static void id_irq_timer(unsigned long _data)
{

	//struct palmas_irq_chip_data *d = container_of(
		//to_delayed_work(work), struct palmas_irq_chip_data, works[ID_IRQ_INDEX-20]);
	struct palmas_irq_chip_data *d =  (void *)_data;
	printk("%s:=========>\n",__func__);
	if(d->disable_mask&ID_IRQ_FLAG)return;
	
	queue_work(d->irq_workq,&d->works[ID_IRQ_INDEX-20]);
}

static void vbus_otg_irq_timer(unsigned long _data)
{
	//struct palmas_irq_chip_data *d = container_of(
			//to_delayed_work(work), struct palmas_irq_chip_data, works[VBUS_OTG_IRQ_INDEX-20]);
	struct palmas_irq_chip_data *d = (void *)_data;
	printk("%s:=========>\n",__func__);
	if(d->disable_mask&VBUS_OTG_IRQ_FLAG)return;

	queue_work(d->irq_workq,&d->works[VBUS_OTG_IRQ_INDEX-20]);

}
static void vbus_irq_timer(unsigned long _data)
{
	//struct palmas_irq_chip_data *d = container_of(
			//to_delayed_work(work), struct palmas_irq_chip_data, works[VBUS_IRQ_INDEX-20]);
	struct palmas_irq_chip_data *d = (void *)_data;
	printk("%s:=========>\n",__func__);
	if(d->disable_mask&VBUS_IRQ_FLAG)return;
	queue_work(d->irq_workq,&d->works[VBUS_IRQ_INDEX-20]);
			...
			int irq_base, struct palmas_irq_chip_data **data)
{
	...
	*data = d;
	...
	d->irq_workq = create_singlethread_workqueue("palmas_irq");
    if (d->irq_workq == NULL){
        pr_err(" palmas - create_workqueue(  palmas_irq  ) FAIL !!! \n");
        //ret = -ENXIO;
    }	

	INIT_WORK(&d->works[ID_OTG_IRQ_INDEX-20],id_ota_irq_work);
	INIT_WORK(&d->works[ID_IRQ_INDEX-20],id_irq_work);
	INIT_WORK(&d->works[VBUS_OTG_IRQ_INDEX-20],vbus_otg_irq_work);
	INIT_WORK(&d->works[VBUS_IRQ_INDEX-20],vbus_irq_work);

	setup_timer(&d->timer[ID_OTG_IRQ_INDEX-20],
			    id_ota_irq_timer, (unsigned long)d);
	setup_timer(&d->timer[ID_IRQ_INDEX-20],
			    id_irq_timer, (unsigned long)d);
	setup_timer(&d->timer[VBUS_OTG_IRQ_INDEX-20],
			    vbus_otg_irq_timer, (unsigned long)d);
	setup_timer(&d->timer[VBUS_IRQ_INDEX-20],
			    vbus_irq_timer, (unsigned long)d);
...
int palmas_powerkey_state(void)
{
	int ret;
	unsigned int reg, addr;
	int slave;
	if(!palmas_dev)return -EINVAL;
	slave = PALMAS_BASE_TO_SLAVE(PALMAS_INTERRUPT_BASE);
	addr = PALMAS_BASE_TO_REG(PALMAS_INTERRUPT_BASE, PALMAS_INT1_LINE_STATE);
	//pr_warn("%s:============>slave=0x%x,addr=0x%x\n",__func__,slave,addr);
	
	if(!palmas_dev->regmap[slave])
		dev_err(palmas_dev->dev,"%s:get %d regmap is NULL\n",__func__,slave);
	ret = regmap_read(palmas_dev->regmap[slave], addr, &reg);
	if (ret)
		return -EINVAL;
	//pr_warn("%s:============>read 0x12=0x%x\n",__func__,reg);
	return reg&(1<<1)?1:0;
}
EXPORT_SYMBOL_GPL(palmas_powerkey_state);
...
EXPORT_SYMBOL(palmas_power_off);
...
static int palmas_i2c_remove(struct i2c_client *i2c)
{
	...
	if(palmas->irq_chip_data){
		if(palmas->irq_chip_data->irq_workq)
			destroy_workqueue(palmas->irq_chip_data->irq_workq);
