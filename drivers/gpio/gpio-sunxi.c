        ...
        struct device_attribute *attr, char *buf)
{
    ...
    gpio_sw_cdev->item->mul_sel = gpio_sw_cdev->gpio_sw_drv_get(gpio_sw_cdev);
...
static int gpio_sw_probe(struct platform_device *dev)
{
	...
	sw_gpio_entry->class.item = kzalloc(sizeof(struct gpio_config), GFP_KERNEL);
	if(!sw_gpio_entry->class.item) {
		kfree(sw_gpio_entry);
		return -ENOMEM;
	}
	//printk("gpio_name: %s\n", pdata->name);
    gpio = of_get_named_gpio_flags(node, pdata->name, 0, (enum of_gpio_flags *)sw_gpio_entry->class.item);
    ...
    ret = mapGpioToName(io_area,sw_gpio_entry->class.item->gpio);
    ...
    gpio_sw_cfg_set(&sw_gpio_entry->class, sw_gpio_entry->class.item->mul_sel);
    gpio_sw_data_set(&sw_gpio_entry->class, sw_gpio_entry->class.item->data);
...
static void __exit gpio_sw_exit(void)
{
    ...
    for(i=0;i<cnt;i++){
    	...
    	gpio = of_get_named_gpio_flags(node, gpio_name, 0, (enum of_gpio_flags *)&config);
    	if (!gpio_is_valid(gpio)){
    		printk(KERN_ERR "this gpio is invalid: %d\n", gpio);
    		continue;	
    	}
...
static int __init sunxi_init_gpio_probe(struct platform_device *pdev)
{
    ...
    for(i=0;i<cnt;i++){
    	...
    	printk("gpio_pin_%d(%d) gpio_is_valid\n",i+1, config.gpio);
        ...
        if (normal_led_pin_str && !strcmp(sw_pdata[i]->name, normal_led_pin_str)) {
            sprintf(sw_pdata[i]->link,"%s", "normal_led");
        } else if (standby_led_pin_str && !strcmp(sw_pdata[i]->name, standby_led_pin_str)) {
...
INIT_END:
    printk("gpio_init finish with uesd \n");
    return 0;
INIT_ERR_FREE:
