...
static struct of_device_id sunxi_mci_dt_ids[] = {
	{ .compatible = "allwinner,sun50i-sdmmc0", .data = &sunxi_mci_pdata },
	{ /* Sentinel */ },
};
//static struct of_device_id sunxi_mci_dt_ids[] = {
...
static void dumphex32(struct sunxi_mmc_host* smc_host, char* name, char* base, int len)
{
	...
	for (i=0; i<len; i+=4) {
		if (!(i&0xf))
			printk("\n0x%p : ", base + i);
		printk("0x%08x ",__raw_readl(smc_host->reg_base+i));
	}
	printk("\n");
}
...
static void sunxi_mci_send_cmd(struct sunxi_mmc_host* smc_host, struct mmc_command* cmd)
{
	...
	SMC_DBG(smc_host, "smc %d cmd %d(%08x) arg %x ie 0x%08x wt %x len %d\n",
		smc_host->host_id, cmd_val&0x3f, cmd->arg, cmd_val, imask, wait,
...
static void sunxi_mci_init_idma_des(struct sunxi_mmc_host* smc_host, struct mmc_data* data)
{
	...
	for (i=0; i<data->sg_len; i++) {
		...
		for (j=0; j < buff_frag_num; j++, des_idx++) {
			...
			if (buff_frag_num > 1 && j != buff_frag_num-1){
				pdes[des_idx].data_buf1_sz = SDXC_DES_BUFFER_MAX_LEN;
				if(pdes[des_idx].data_buf1_sz != SDXC_DES_BUFFER_MAX_LEN){
					SMC_ERR(smc_host,"Write des failed\n");
				}
			}else{
				pdes[des_idx].data_buf1_sz = remain;
				if(pdes[des_idx].data_buf1_sz != remain){
					SMC_ERR(smc_host,"Write des failed\n");
				}
			}
			...
			pdes[des_idx].buf_addr_ptr1 = sg_dma_address(&data->sg[i])
							+ j * SDXC_DES_BUFFER_MAX_LEN;
			if(pdes[des_idx].buf_addr_ptr1 != (sg_dma_address(&data->sg[i])
							+ j * SDXC_DES_BUFFER_MAX_LEN)){
					SMC_ERR(smc_host,"Write des failed\n");
			}
			...
			if ((i == data->sg_len-1) && (j == buff_frag_num-1)) {
				...
				if(pdes[des_idx].buf_addr_ptr2 != 0){
					SMC_ERR(smc_host,"Write des failed\n");
				}
				...
				pdes[des_idx].buf_addr_ptr2 = (u32)&pdes_pa[des_idx+1];
				if(pdes[des_idx].buf_addr_ptr2 != (u32)&pdes_pa[des_idx+1]){
					SMC_ERR(smc_host,"Write des failed\n");
				}
				
			}
			pdes[des_idx].config = config;
			if(pdes[des_idx].config != config){
				SMC_ERR(smc_host,"Write des failed\n");
			}
			SMC_INFO(smc_host, "sg %d, frag %d, remain %d, des[%d](%08x): "
					"[0] = %08x, [1] = %08x, [2] = %08x, [3] = %08x\n", i, j, remain,
...
static int 
	sunxi_mci_prepare_dma(struct sunxi_mmc_host* smc_host, struct mmc_data* data)
{
	u32 dma_len;
	u32 i;
	u32 temp;
...
void sunxi_mci_dump_errinfo(struct sunxi_mmc_host* smc_host)
{
	...
	SMC_ERR(smc_host, "smc %d err, cmd %d, %s%s%s%s%s%s%s%s%s%s !!\n",
		smc_host->host_id, smc_host->mrq->cmd ? smc_host->mrq->cmd->opcode : -1,
		...
		//dumphex32(smc_host, "mmc", IOMEM(IO_ADDRESS(SMC_BASE(smc_host->host_id))), 0x100);
		//dumphex32(smc_host, "mmc", IO_ADDRESS(SMC_BASE(smc_host->host_id)), 0x100);
		//if(smc_host->int_sum & SDXC_FIFORunErr)
		
		{
		//	dumphex32(smc_host, "mmc", SMC_BASE(smc_host->host_id), 0x100);
		//mclk_base = __io_address(SUNXI_CCM_BASE + 0x88 + 0x4 * smc_host->host_id);
		//dumphex32(smc_host, "ccmu", mclk_base, 0x4);
		//dumphex32(smc_host, "gpio", IO_ADDRESS(SUNXI_PIO_BASE), 0x120);

		//	SMC_MSG(smc_host, "des(%08x): "
		//			"[0] = %08x, [1] = %08x, [2] = %08x, [3] = %08x\n", 
		//		 (u32)&pdes[0],
		//		(u32)((u32*)&pdes[0])[0], (u32)((u32*)&pdes[0])[1],
		//		(u32)((u32*)&pdes[0])[2], (u32)((u32*)&pdes[0])[3]);
		}
...
{
	void __iomem *mclk_base = __io_address(SUNXI_CCM_BASE + 0x88 + 0x4 * smc_host->host_id);
	if (enable) {
...
//	struct sunxi_mmc_clk_dly* mmc_clk_dly = smc_host->pdata->mmc_clk_dly;
	...
	*/
	...
	//clk_set_rate(mclk, sdmmc2mod_rate); 
	//clk_put(mclk2);
...
//#endif
	...
	clk = smc_host->card_clk = rate/2;
	smc_host->mod_clk = rate;
...
//	}
	...
	if (smc_host->ddr) {
		...
		smc_host->card_clk = smc_host->mod_clk / 4;
...
/*
	if ((smc_host->host_id == 2) && (smc_host->pdata->used_2xmod == 1))
	{
...
//	struct resource* res = NULL;
	...
	smc_host->reg_base =  of_iomap(np, 0);
...
static int sunxi_mci_resource_release(struct sunxi_mmc_host *smc_host)
{
	...
	iounmap(smc_host->reg_base);
	release_mem_region(SMC_BASE(smc_host->host_id), SMC_BASE_OS);
...
static irqreturn_t sunxi_mci_irq(int irq, void *dev_id)
{
	...
	if (!msk_int && !idma_int) {
		SMC_DBG(smc_host, "sdc%d nop irq: ri %08x mi %08x ie %08x idi %08x\n",
			smc_host->host_id, raw_int, msk_int, idma_inte, idma_int);
	...
	SMC_INFO(smc_host, "smc %d irq, ri %08x(%08x) mi %08x ie %08x idi %08x\n",
		smc_host->host_id, raw_int, smc_host->int_sum,
	...
	if (smc_host->wait == SDC_WAIT_NONE && !sdio_int) {
		SMC_ERR(smc_host, "smc %x, nothing to complete, ri %08x, "
			"mi %08x\n", smc_host->host_id, raw_int, msk_int);
		goto irq_out;
	}
...
static void sunxi_mci_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	SMC_MSG(smc_host, "sdc%d set ios: "
		...
		smc_host->host_id, ios->clock, bus_mode[ios->bus_mode],
...
static int sunxi_mci_execute_tuning(struct mmc_host *mmc, u32 opcode)
{
	...
	if (sample_win) {
		...
		SMC_MSG(smc_host, "sdc%d sample_window:[%d, %d], sample_point %d\n",
				smc_host->host_id, sample_min, sample_max, sample_dly);
		sunxi_mci_set_clk_dly(smc_host, smc_host->oclk_dly, sample_dly);
		err = 0;
	} else {
...
static int sunxi_mci_proc_drvversion_show(struct seq_file *m, void *v)
{
	seq_printf(m, "%s\n",DRIVER_VERSION);
	return 0;
}
...
static int
sunxi_mci_proc_drvversion_open(struct inode *inode, struct file *file)
{
	return single_open(file, sunxi_mci_proc_drvversion_show, NULL);

}

static const struct file_operations sunxi_mci_proc_drvversion_fops = {
	.owner = THIS_MODULE,
	.open  = sunxi_mci_proc_drvversion_open,
	.read  = seq_read,
	.llseek    = seq_lseek,
	.release   = single_release,
};

static int sunxi_mci_proc_hostinfo_show(struct seq_file *m, void *v)
{

	struct sunxi_mmc_host *smc_host = (struct sunxi_mmc_host *)m->private;
	struct device* dev = &smc_host->pdev->dev;
	char* cd_mode[] = {"None", "GPIO Check", "GPIO IRQ", "Always In", "Manual", "DATA3"};
	char* state[] = {"Idle", "Sending CMD", "CMD Done"};
	char* vol[] = {"3.3V", "1.8V", "1.2V", "off"};
	...
	seq_printf(m, " %s Host Info:\n", dev_name(dev));
	seq_printf(m, " REG Base  : %p\n", smc_host->reg_base);
	seq_printf(m, " DMA Desp  : %p(%08x)\n", smc_host->sg_cpu, smc_host->sg_dma);
	seq_printf(m, " Mod Clock : %d\n", smc_host->mod_clk);
	seq_printf(m, " Card Clock: %d\n", smc_host->card_clk);
	seq_printf(m, " Oclk Delay: %d(%d.%dns)\n", smc_host->oclk_dly, odly/10, odly%10);
	seq_printf(m, " Sclk Delay: %d(%d.%dns)\n", smc_host->sclk_dly, sdly/10, odly%10);
	seq_printf(m, " Bus Width : %d\n", smc_host->bus_width);
	seq_printf(m, " DDR Mode  : %d\n", smc_host->ddr);
	seq_printf(m, " 2xCLK Mode: %d\n", smc_host->pdata->used_ddrmode);
	seq_printf(m, " Voltage   : %s\n", vol[smc_host->voltage]);
	seq_printf(m, " Present   : %d\n", smc_host->present);
	seq_printf(m, " CD Mode   : %s\n", cd_mode[smc_host->cd_mode]);
	seq_printf(m, " Read Only : %d\n", smc_host->read_only);
...
static int
sunxi_mci_proc_hostinfo_open(struct inode *inode, struct file *file)
{
	return single_open(file, sunxi_mci_proc_hostinfo_show, PDE_DATA(inode));

}

static const struct file_operations sunxi_mci_proc_hostinfo_fops = {
	.owner = THIS_MODULE,
	.open  = sunxi_mci_proc_hostinfo_open,
	.read  = seq_read,
	.llseek    = seq_lseek,
	.release   = single_release,
};
...
static int sunxi_mci_proc_read_regs_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "Dump smc regs:\n");
	for (i=0; i<0x100; i+=4) {
		if (!(i&0xf))
			seq_printf(m, "\n0x%08x : ", (u32)(smc_host->reg_base + i));
		seq_printf(m, "%08x ", readl(smc_host->reg_base + i));
	}
	seq_printf(m, "\n");
...
static int
sunxi_mci_proc_read_regs_open(struct inode *inode, struct file *file)
{
	return single_open(file, sunxi_mci_proc_read_regs_show, PDE_DATA(inode));
}

static const struct file_operations sunxi_mci_proc_read_regs_fops = {
	.owner = THIS_MODULE,
	.open  = sunxi_mci_proc_read_regs_open,
	.read  = seq_read,
	.llseek    = seq_lseek,
	.release   = single_release,
};
...
/*
static int sunxi_mci_proc_rw_dbglevel_show(struct seq_file *m, void *v)
{
...
static int
sunxi_mci_proc_rw_dbglevel_open(struct inode *inode, struct file *file)
{
	return single_open(file, sunxi_mci_proc_rw_dbglevel_show, PDE_DATA(inode));
}
...
ssize_t sunxi_mci_proc_rw_dbglevel_read(struct file *file, char __user *buf, size_t size, loff_t *ops)
{
	u32 dtest = 3;

	SMC_MSG(NULL, "*** %s(L%d),%d\n", __FUNCTION__, __LINE__,*ops);
	if(*ops>4){
		return -ENXIO;
	}
	...
	copy_to_user(buf,(void *)(&dtest),4);
	SMC_MSG(NULL, "*** %s(L%d)\n", __FUNCTION__, __LINE__);

	return 4;	
}

static const struct file_operations sunxi_mci_proc_rw_dbglevel_fops = {
	.owner = THIS_MODULE,
	.open  = sunxi_mci_proc_rw_dbglevel_open,
	.read  = sunxi_mci_proc_rw_dbglevel_read,
	.llseek    = seq_lseek,
	.release   = single_release,
};
*/
...
void sunxi_mci_procfs_attach(struct sunxi_mmc_host *smc_host)
{
	...
	smc_host->proc_drvver = proc_create("drv-version", 0444,
				smc_host->proc_root, &sunxi_mci_proc_drvversion_fops);
	if (IS_ERR(smc_host->proc_drvver))
		SMC_MSG(smc_host, "%s: failed to create procfs \"drv-version\".\n", dev_name(dev));


	smc_host->proc_hostinfo = proc_create_data("hostinfo", 0444,
				smc_host->proc_root, &sunxi_mci_proc_hostinfo_fops,smc_host);
	if (IS_ERR(smc_host->proc_hostinfo))
		SMC_MSG(smc_host, "%s: failed to create procfs \"hostinfo\".\n", dev_name(dev));


	smc_host->proc_regs = proc_create_data("register", 0444,
				smc_host->proc_root, &sunxi_mci_proc_read_regs_fops, smc_host);
	if (IS_ERR(smc_host->proc_regs))
		SMC_MSG(smc_host, "%s: failed to create procfs \"hostinfo\".\n", dev_name(dev));

/*

	smc_host->proc_dbglevel = proc_create_data("debug-level", 0644, smc_host->proc_root,&sunxi_mci_proc_rw_dbglevel_fops,smc_host);
...
static int sunxi_mci_probe(struct platform_device *pdev)
{
	...
	of_id = of_match_device(sunxi_mci_dt_ids, &pdev->dev);

	//SMC_MSG(NULL, "*** %s(L%d)\n", __FUNCTION__, __LINE__);


	if (!of_id || !of_id->data) {
		dev_err(&pdev->dev, "Controller pdata data missing\n");
		return -EFAULT;
	}
	...
	smc_host->irq = irq_of_parse_and_map(np, 0);
	...
	sunxi_host[smc_host->host_id] = smc_host;
...
static struct platform_driver sunxi_mci_driver = {
	...
	.driver.of_match_table = sunxi_mci_dt_ids,
