...
#define INTEL_SOFIA
//#define INTEL_DEBUG_JINHUIJIE
#ifdef INTEL_DEBUG_JINHUIJIE
#define PRINT_DEBUG_JINHUIJIE(...) \
          printk(__VA_ARGS__)
#else
#define PRINT_DEBUG_JINHUIJIE(...)
#endif
...
#define CHECK_CHIP_ID_TIME_MAX 10
...
#ifdef INTEL_DEBUG_JINHUIJIE
#define BREAK_JINHUIJIE  break
#else
#define BREAK_JINHUIJIE
#endif
					...
					struct bma2x2_data *data)
{
	...
	while (read_count++ < CHECK_CHIP_ID_TIME_MAX) {
		if (err = bma2x2_smbus_read_byte(client, BMA2X2_CHIP_ID_REG,
							&chip_id) < 0) {
		...
		const struct i2c_device_id *id)
{
	...
	if (!data) {
        ...
        #ifdef CONFIG_ACPI
        pr_info("%s, i2c addr from acpi is 0x%x\n",__func__,client->addr);
        client->addr = 0x18; 
        #endif
...
#ifdef CONFIG_ACPI
static const struct acpi_device_id fake_kionix_acpi_match[] = {
	{"KIOX0009", 0},
	{ }
};
static const struct acpi_device_id bma250e_acpi_match[] = {
	{"BMA250E0", 0},
	{ }
};
...
#endif
#ifdef CONFIG_ACPI
		.acpi_match_table = ACPI_PTR(bma250e_acpi_match),
#endif
#endif
	},
