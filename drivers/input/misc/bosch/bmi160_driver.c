...
static int bmi_input_init(struct bmi_client_data *client_data)
{
	...
	if (err < 0) {
		input_free_device(dev);
		dev_notice(client_data->dev, "bmi160 gyro input free!\n");
		return err;
	}
	client_data->input2 = dev;
	dev_notice(client_data->dev,
		"bmi160 gyro  input register successfully, %s!\n",
		client_data->input2->name);
...
#ifdef INTEL_SENSORS
static void bmi_work_func2(struct work_struct *work)
{
	struct bmi_client_data *client_data =
		container_of((struct delayed_work *)work,
			struct bmi_client_data, work2);
	unsigned long delay =
		msecs_to_jiffies(atomic_read(&client_data->delay2));
	struct bmi160_accel_t data;
	struct bmi160_axis_data_t bmi160_udata;
	int err;
	...
	pr_info("%s,%d,:%d,%d,%d",__func__,__LINE__,data.x,data.y,data.z);
	bmi_remap_sensor_data(&bmi160_udata, client_data);
	/*report current frame via input event*/
	input_event(client_data->input2, EV_REL, REL_X, bmi160_udata.x);
	input_event(client_data->input2, EV_REL, REL_Y, bmi160_udata.y);
	input_event(client_data->input2, EV_REL, REL_Z, bmi160_udata.z);
	input_sync(client_data->input2);

	schedule_delayed_work(&client_data->work2, delay);
}
...
static ssize_t bmi160_enable2_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct input_dev *input = to_input_dev(dev);
	struct bmi_client_data *client_data = input_get_drvdata(input);

	return sprintf(buf, "%d\n", atomic_read(&client_data->wkqueue_en2));

}

static ssize_t bmi160_enable2_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct input_dev *input = to_input_dev(dev);
	...
	int err;
	unsigned long enable;
	int pre_enable = atomic_read(&client_data->wkqueue_en2);
	...
	if (enable) {
		if (pre_enable == 0) {
			...
			schedule_delayed_work(&client_data->work2,
			msecs_to_jiffies(atomic_read(&client_data->delay2)));
			atomic_set(&client_data->wkqueue_en2, 1);
		}
		...
		if (pre_enable == 1) {
			...
			cancel_delayed_work_sync(&client_data->work2);
			atomic_set(&client_data->wkqueue_en2, 0);
		}
	}
...
static ssize_t bmi160_delay2_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct input_dev *input = to_input_dev(dev);
	struct bmi_client_data *client_data = input_get_drvdata(input);

	return sprintf(buf, "%d\n", atomic_read(&client_data->delay2));

}

static ssize_t bmi160_delay2_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct input_dev *input = to_input_dev(dev);
	...
	atomic_set(&client_data->delay2, (unsigned int)data);

	return count;
}

static DEVICE_ATTR(enable2, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi160_enable2_show, bmi160_enable2_store);
static DEVICE_ATTR(delay2, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmi160_delay2_show, bmi160_delay2_store);


static struct attribute *bmi160_attributes2[] = {
	&dev_attr_enable2.attr,
	&dev_attr_delay2.attr,
	NULL
};

static struct attribute_group bmi160_attribute_group2 = {
	.attrs = bmi160_attributes2
};
#endif
...
int bmi_probe(struct bmi_client_data *client_data, struct device *dev)
{
...
#ifdef  INTEL_SENSORS
	err = sysfs_create_group(&client_data->input2->dev.kobj,
			&bmi160_attribute_group2);
	...
	atomic_set(&client_data->wkqueue_en, 0);
#ifdef INTEL_SENSORS
	INIT_DELAYED_WORK(&client_data->work2, bmi_work_func2);
	atomic_set(&client_data->delay2, BMI_DELAY_DEFAULT);
	atomic_set(&client_data->wkqueue_en2, 0);
