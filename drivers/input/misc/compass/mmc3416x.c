...
static int mmc_nvi_mpu_bypass_request()
{
	int i;
	int err = 0;
...
static int mmc_nvi_mpu_bypass_release()
{
	int err = 0;
...
static int mmc3xxx_i2c_rx_data(char *buf, int len)
{
	...
	err = mmc_nvi_mpu_bypass_request();
  ...
  if(!err){
		...
		mmc_nvi_mpu_bypass_release();
  }
  else
  {
  		printk("nvi mpu bypass request fail,write!");
  }
	return 0;
}
...
static int mmc3xxx_i2c_tx_data(char *buf, int len)
{
  ...
  {
  		printk("nvi mpu bypass request fail,read!");
  }
	return 0;
}
...
static long mmc3416x_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case MMC3416X_IOC_DIAG:
		...
		printk("MMC_3416X_IOC_DIAG data:%d",data[0]);
...
static int mmc_pm_init(struct i2c_client *client)
{
	unsigned int i;
	int err = 0;

	struct regulator * reg_osc = regulator_get(&client->dev, "vdd");
	struct regulator * reg_vda = regulator_get(&client->dev, "vda");
    regulator_enable(reg_osc);
    regulator_enable(reg_vda);
	return err;
}
