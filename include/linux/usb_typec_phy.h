...
struct typec_ops {
	...
	int (*measure_cc)(struct typec_phy *phy, int pin,
			struct typec_cc_psy *cc_psy, unsigned long timeout);
	int (*switch_mode)(struct typec_phy *phy, enum typec_mode mode);
	int (*enable_cc_pullup)(struct typec_phy *phy);
	int (*enable_cc_pulldown)(struct typec_phy *phy);
...
struct typec_phy {
	...
	int (*is_pd_capable)(struct typec_phy *phy);
...
extern int typec_get_cc_orientation(void);
...
static inline int typec_enable_cc_pullup(struct typec_phy *phy)
{
	if (phy && phy->ops.enable_cc_pullup)
		return phy->ops.enable_cc_pullup(phy);
	return 0;
}

static inline int typec_enable_cc_pulldown(struct typec_phy *phy)
{
	if (phy && phy->ops.enable_cc_pulldown)
		return phy->ops.enable_cc_pulldown(phy);
	return 0;
}
