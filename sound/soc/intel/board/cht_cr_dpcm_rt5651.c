/*
 *  cht_cr_dpcm_rt5651.c - ASoc DPCM Machine driver
								...
								int jack_type)
{
	...
	case SND_JACK_HEADSET:
		...
		pr_info("Setting the micbias for %s\n", board_name);
		if (strcmp(board_name, "Cherry Trail CR") == 0) //Cherry Trail FFD
	...
	case SND_JACK_HEADPHONE:
		...
		cht_force_enable_pin(codec, "micbias1", false); //micbias2
		break;
	case 0:
					...
					struct snd_soc_codec *codec)
{
	...
	status = rt5651_check_jd_status(codec);
	...
	if (!status) {
		...
		if (strcmp(board_name, "Cherry Trail Tablet") == 0) {
			rt5651_supported_hs_type(codec,
					RT5651_HS_RING4_MICBIAS2);
		} else if (strcmp(board_name, "Cherry Trail FFD") == 0) {
			rt5651_supported_hs_type(codec,
					RT5651_HS_RING4_MICBIAS1);
		}
#endif
		jack_type = rt5651_headset_detect(codec, true);
		printk(KERN_ERR "status = %d jack_type: %x    %s \n", status, jack_type, __func__);
		switch (jack_type) {
...
static void cht_check_hs_button_status(struct work_struct *work)
{
	...
	if (((jack->status & SND_JACK_HEADSET) == SND_JACK_HEADSET)
		...
		if (!status) {
			...
			status = rt5651_check_bp_status(codec);
...
//	snd_soc_update_bits(codec, RT5651_JD_CTRL,
...
{
	pr_info("Cherrytrail Machine Driver cht_rt5651 registerd\n");
...
MODULE_ALIAS("platform:cht_rt5651");
