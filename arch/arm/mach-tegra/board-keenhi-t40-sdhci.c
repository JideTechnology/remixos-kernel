/*
 * arch/arm/mach-tegra/board-keenhi_t40-sdhci.c
 *
...
static int keenhi_t40_wifi_status_register(void (*callback)(int , void *), void *);

static int keenhi_t40_wifi_power(int on);
static int keenhi_t40_wifi_set_carddetect(int val);

#if 0
static struct wl12xx_platform_data keenhi_t40_wl12xx_wlan_data __initdata = {
	.board_ref_clock = WL12XX_REFCLOCK_26,
	.board_tcxo_clock = 1,
	.set_power = keenhi_t40_wifi_power,
	.set_carddetect = keenhi_t40_wifi_set_carddetect,
#if defined(CONFIG_WLCORE_EDP_SUPPORT)
	.edp_info = {
		.client_info = {
			.name = "wl_edp_client",
...
static struct tegra_sdhci_platform_data tegra_sdhci_platform_data0 = {
	.mmc_data = {
		.register_status_notify	= keenhi_t40_wifi_status_register,
#ifdef CONFIG_MMC_EMBEDDED_SDIO
...
static int keenhi_t40_wifi_status_register(
...
static int __init keenhi_t40_wifi_init(void)
{
	int rc;
#if 0
	...
	keenhi_t40_wl12xx_wlan_data.irq = gpio_to_irq(MACALLAN_WLAN_WOW);
	wl12xx_set_platform_data(&keenhi_t40_wl12xx_wlan_data);
#else
	/* init wlan_pwr gpio */
	rc = gpio_request(MACALLAN_WLAN_PWR, "wlan_power");
...
static int __init keenhi_t40_wifi_prepower(void)
{
	keenhi_t40_wifi_power(1);

	return 0;
}

subsys_initcall_sync(keenhi_t40_wifi_prepower);
#endif

int __init keenhi_t40_sdhci_init(void)
{
	int nominal_core_mv;
